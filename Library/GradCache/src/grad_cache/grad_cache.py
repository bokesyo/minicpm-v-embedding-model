...
class GradCache:
    ...
    def compute_loss(self, *reps: Tensor, **loss_kwargs) -> [Tensor,Tensor]:
        """
        ...
        """
        loss,accuracy = self.loss_fn(*reps, **loss_kwargs)
    ...
    def build_cache(self, *reps: Tensor, **loss_kwargs) -> [List[Tensor], Tensor, Tensor]:
        """
        ...
        with autocast() if self.fp16 else nullcontext():
            loss,accuracy = self.compute_loss(*reps, **loss_kwargs)
        ...
        return cache, loss.detach(),accuracy
    ...
    def cache_step(
        ...
        cache, loss, accuracy = self.build_cache(*all_reps, **loss_kwargs)
