...
for d in dataset_files:
    if d.endswith(".jsonl"):
        ...
        continue
    if not d.startswith('marco'):
        continue
    # if not (d.startswith('simclue') or d.startswith('retrieval')):
    #     continue
    print("================{}==============".format(d))
    ...
    negs = [(neg+[docs[similarity_q_index[i][j]]for j in range(25,35)])[:10] for neg,i in zip(old_negs,range(len(dataset)))]
