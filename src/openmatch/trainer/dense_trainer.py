...
class DRTrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if state_dict is not None: # with deepspeed training
            ...
            if self.model.base_model_arch == "MiniCPMVForMMEmbedding": # Multimodal embedding
                ...
                logger.warning("model arch not handled, so save the state_dict directly.")
    ...
    def training_step(self, model: torch.nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.grad_cache_enable:
            ...
            num_passages = passage["input_ids"].shape[0] // query["input_ids"].shape[0]
            ...
            with torch.no_grad(): 
                ...
                # hidden_dim=2304, sizeof(bf16)=2, mini_bsz=512, world_size=8(#gpu=8), num_passages=6 then the memory consumption is 108MB for query
                ...
                for i in range(0, num_microbatches):
                    ...
                    micro_passage = BatchEncoding({k: v[i * micro_bsz * num_passages: (i + 1) * micro_bsz * num_passages] for k, v in passage.items()})
                    
                    # step 1b: compute representations of mini-batch of query & passage
                    outputs_ = model(query=micro_query, passage=micro_passage)
            ...
            for i in range(0, num_microbatches):
                ...
                with context:
                    ...
                    with self.compute_loss_context_manager():
                        outputs_ = model(query=micro_query, passage=micro_passage) # <- this model call is with gradient and intermediate states
                        q_reps_ = outputs_.q_reps
                        p_reps_ = outputs_.p_reps
                        ...
                        p_reps_tmp[(i * global_micro_bsz + self.process_rank * micro_bsz) * num_passages: (i * global_micro_bsz + (self.process_rank + 1) * micro_bsz) *  num_passages] = p_reps_
...
class GCDenseTrainer(DRTrainer):
    ...
    def training_step(self, model, inputs) -> torch.Tensor:
        ...
        loss,accuracy = self.gc(queries, passages, no_sync_except_last=_distributed)
        self.metric_hook["accuracy"].append(accuracy.item())
