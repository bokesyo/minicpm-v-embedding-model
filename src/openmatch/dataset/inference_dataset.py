...
def get_idx(obj):
    ...
    # if example_id is None: # maybe image dataset (contains `source` and `page` key)
    #     example_id_source = obj.get("source", None)
    #     example_id_page = obj.get("page", None)
    #     if (example_id_source is not None) and (example_id_page is not None):
    #         example_id = f"{example_id_source}-{example_id_page}"
    if example_id is None:
