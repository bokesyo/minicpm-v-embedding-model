...
GRADCACHE=${13}
GRADCACHE_MICRO=${14}
PASSAGE_GRAD=${15} # by default it is true
CHECKPOINT_MODEL=${16}
GPUS_PER_NODE=8
IN_BATCH=true
LORA=false
...
# bash train_mm.sh 1024 32 0.02 1 true true ds_config_warmup_decay.json 5e-5 false simple_lasttoken bidirectional 1 true 4 true
# bash train_mm.sh 1024 32 0.02 1 true true ds_config_warmup_decay.json 5e-5 false simple_lasttoken bidirectional 1 true 8 false
...
pip install -r /local/apps/openmatch/Research/vision/requirements.txt
...
DATA_FILE_PATH="$DATASET_PATH/data.jsonl"
...
torchrun \
    ...
    --passage_stop_grad $PASSAGE_GRAD \
    --deepspeed $DEEPSPEED \
