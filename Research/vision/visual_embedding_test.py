...
b_path = Path(__file__).parent / 'modeling_minicpmv' # 将modeling_minicpmv目录的路径添加到sys.path中
sys.path.append(str(b_path))
# from modeling_minicpmv import MiniCPMV
# from modeling_minicpmv import LlamaTokenizerWrapper # tokenizer

from openmatch.modeling.modeling_minicpmv.modeling_minicpmv import MiniCPMVForMMEmbedding
    ...
    with amp.autocast() if args.fp16 else nullcontext():
        with torch.no_grad():
            for batch in tqdm(dataloader, disable=args.process_index > 0):
                ...
                # for chat
                msgs = [ prompt ] * len(images_string) # [ChatML] * B
                ...
                # print(response.keys())
                print("last_hidden_state", response["last_hidden_state"].shape)
...
def main():
    ...
    model_cls = MiniCPMVForMMEmbedding
    ...
    distributed_parallel_inference(
        ...
        model=model,
        tokenizer=tokenizer,
        prompt="represent this doc for retrieval:",
