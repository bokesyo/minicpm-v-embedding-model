...
class MiniCPMV(MiniCPMVPreTrainedModel):
    ...
    # def forward(self, data, **kwargs): # 这个forward应该不会被self.generate调用 因为self.generate调用的是self._decode，而self._decode调用的是self.llm.generate...
    ...
    def forward( # 正统 LM forward, modified from self.generate
        self,
        data_list=None, # List[str] raw input
        img_list=None, # List[List[PIL.Image]] raw input
        tokenizer=None,
        max_inp_length: Optional[int] = None,
        vision_hidden_states=None, # default None
        return_vision_hidden_states=False,
        **kwargs
        ...
        # for embedding, we should call llm.forward with inputs_embeds (because image tokens have been merged into inputs_embeds)
        result = self.llm.model.forward(
            input_ids=None, # because image and text have been merged into model_inputs["inputs_embeds"] here.
            position_ids=None,
    ...
    def chat(
        self,
        image_list, # List[ PIL.Image ] B*1
        msgs_list, # List[Dict[str, str]] B*Dict
        context, # depreciated
        tokenizer,
