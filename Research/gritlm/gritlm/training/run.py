...
class CustomTrainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if is_datasets_available() and isinstance(train_dataset, datasets.Dataset):
            train_dataset = self._remove_unused_columns(1, description="training")
        ...
        if isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            # print(f"!!!! dataloader_num_workers = {self.args.dataloader_num_workers}")
...
def filter_too_long_instructions(tokenizer, dataset, query_max_len, passage_max_len):
    ...
    num_proc = 12 if len(dataset) > 5000 else 1
...
def main():
    ...
    # data_files = [os.path.join(data_args.train_data, x) for x in os.listdir(data_args.train_data) 
    #           if x.endswith('.jsonl')] if os.path.isdir(data_args.train_data) else [data_args.train_data]
    ...
    #     tmp_ds = datasets.load_dataset('json', data_files=file, split='train', streaming=True)
    ...
    if training_args.mode == "embedding":
        ...
        # ds = datasets.load_dataset('json', data_files=data_args.train_data, split='train', streaming=True)
        
        # world_size = dist.get_world_size()
        # process_rank = dist.get_rank()
        
        # # wrap one layer, still a Dataset object, but different from process to process.
        # train_dataset = IterableDatasetShard(
        #     ds,
        #     batch_size=training_args.per_device_train_batch_size,
        #     drop_last=True,
        #     num_processes=world_size, # this world_size is global (multi gpu and multi nodes).
        #     process_index=process_rank, # this process_rank is global (multi gpu and multi nodes).
        # )
        
        # streaming or not
        train_dataset_cls = (
            MappingDRTrainDataset if data_args.use_mapping_dataset else StreamDRTrainDataset
        )
