...
class LM:
    ...
    def get_perplexity(self, input_texts: str or List, batch: int = None):
        ...
        # what does this do? 
        # it creates a list of tuples, where each tuple is a pair of indices
        # what if last batch is smaller than batch size? 
        # 
        ...
        with torch.no_grad():
            for s, e in tqdm(batch_id):
                ...
                shift_label = label[:, 1:].contiguous().to(self.device) # label move to device here 
